...
ImageReducer.supportedOptions = {
		...
		directColorBitDepth				: { type: 'int', options: [ 1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 16 ] },
...
ImageReducer.prototype = {
	...
	eventProcessFile : function()
	{
		...
		return function( fileInfo, next )
		{
			...
			if( ( fileInfo.extensionLower === '.svg' ) && ( reducer.options.gzipSvg === true ) )
			{
	...
	manageImage : function( fileInfo, next )
	{
		...
		if( this.options.imagick )
		{
			im.options( { imageMagick: true } );
		}
		...
		im.identify(
				function( err, imInfo )
				{
					...
					var targetFormat = reducer.determineOutputFormat( fileInfo, imInfo );
	...
	/**
	 * Figure out which format the image should be saved as
	 * @param {object} fileInfo
	 * @param {object} imInfo
	 * @returns {string}
	 * @private
	 */
	determineOutputFormat : function( fileInfo, imInfo )
	{
		// Target output format
		var targetFormat = fileInfo.extension;

		// Force direct color PNGs to JPGs?
		if( ( !imInfo.color ) && ( fileInfo.extensionLower === '.png' ) && ( this.options.forcePngToJpg ) )
		{
			targetFormat = '.jpg';
		}

		// Convert SVGs to PNGs?
		if( ( fileInfo.extensionLower === '.svg' ) && ( this.options.forceSvgToPng ) )
		{
			targetFormat = '.png';
		}

		// Force output format?
		if( ( !imInfo.color ) && ( this.options.forceDirectColorOutputFormat ) )
		{
			targetFormat = '.' + this.options.forceDirectColorOutputFormat;
		}
		else if( ( imInfo.color ) && ( this.options.forceIndexedColorOutputFormat ) )
		{
			targetFormat = '.' + this.options.forceIndexedColorOutputFormat;
		}
		...
		return function( err )
		{
			...
			fs.stat(
					...
					function( err, finalFileInfo )
					{
						...
						if(
								( finalFileInfo.size < fileInfo.size ) ||
								(
										( !reducer.options.flexibleFormat ) &&
										( path.extname( finalFile ).toLowerCase() !== path.extname( fileInfo.fullPath ).toLowerCase() )
								)
						)
						{
							reducer.stats.data.optimized += finalFileInfo.size;
	...
	manageColorDepth : function( im, imInfo, fileInfo )
	{
		...
		if(
			...
			// SVGs that are converted to PNGs
			( ( fileInfo.extension === '.svg' ) && ( this.options.forceSvgToPng ) )
		)
		{
